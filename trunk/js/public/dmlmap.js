...
TABLE OF CONTENTS 
	...
	3. MARKER FUNCTIONS
		3.1 - Marker 
		3.2 - Marker prototype 
		3.3 - Marker Label
		3.4 - dml_add_Cluster_Markers()		Adds temporary marker when clicked on the map
		3.5 - dml_Container_Path()    		Draws marker cntainers
		3.6 - dml_delete_Markers() 			Clears markers 
		3.6.1 dml_clear_Markers()			
		3.7 - dml_set_Map_On_All()			Sets the map on all markers in the array.
		3.8 - dml_add_for_distance() 		Add marker for distance measurement
		3.9 - dml_Compute_Distance() 		Calculates distance and drwas lines
		3.10- dml_toRad() 					Helper function to calculate distance
		3.11- dml_Reset_Distance_Elements() Resets distance compute markers and route
		3.12- dml_add_Marker() 				Creates temp marker to calculate distance 
		3.13- dml_Clear_Temp_Marker			Clear temporary marker from the map 
	...
	4. HELPER FUNCTIONS 
		...
		4.4 - dmlShapeInfoWindowContent()	Writes public info windows of the shapes  
		4.5 - dmlWriteShapeInfoImageHelper()  Writes image link public info windows of the shapes 
		4.6 - dmlWriteShapeInfoVideoHelper()  Writes video link public info windows of the shapes 
		4.7 - dmlWriteShapeInfoLink() 		  Writes URL text and value public info windows of the shapes
*/
...
function Load_Dml_Map_Api(myApi) {
	...
	script.src = "https://maps.googleapis.com/maps/api/js?key=" + myApi + "&callback=dml_Load_Map";
	script.id = "dmlMapApi";
...
// 3.4	- Adds temporary marker when clicked on the map
...
// 3.8	- Add marker for distance measurement
...
// 3.9	- Calculates distance and draws lines
...
// 3.10	- Helper function to calculate distance
function dml_toRad(deg) {
	return deg * Math.PI / 180;
}
// 3.11 	- Resets distance compute markers and route 
...
// 3.12	- Creates temp marker to calculate distance 
...
// 3.13	- Clear temporary marker from the map 
