...
TABLE OF CONTENTS 
	...
	3. MARKER FUNCTIONS
		...
		3.2 - Marker 						Custom Marker 
		3.3 - Marker.prototype.setMap 		Custom marker set map
		3.4 - MarkerLabel 					Marker Label Overlay
		3.5 - dml_add_Marker() 				Creates new pin on the map 
		3.6 - dml_delete_Markers()
		      dml_clear_Markers()			Clears markers from the map 
		3.7 - dml_set_Map_On_All() 			Sets the map on all markers in the array.
		3.8 - dml_Center_Map() 				Centers map 
		3.9 - dml_Create_New_Marker() 		Creates new marker and shape on the map 
		3.10- dml_Find_From_Address() 		Finds and shows temp marker on the map based on the address
	...
	5. HELPER FUNCTIONS
		...
		5.2 - dmlMapTypeString() 			Activates text field for the Map Style code
		5.3 - dml_Clear_All_Db_Markers() 	Clears all markers from the map
		5.4 - dml_Select_One_icon() 		Sets icon ID when a new icon selected for the marker
		5.5 - dml_Select_Icon_Container()   Sets icon container ID when a new icon selected for the marker
		5.6 - dml_Select_Icon_Container_Color() Sets container color
		5.7 - dml_Delete_Db_Marker() 		Calls AJAX to delete selected marker from the database
		5.8 - dml_solve_Coordinates_Lat()	Solves Lat data of the shape
		5.9 - dml_solve_Coordinates_Lng() 	Solves Lng data of the shape
...
function Load_Dml_Map_Api(myApi) {
	...
	script.src = "https://maps.googleapis.com/maps/api/js?key=" + myApi + "&libraries=drawing&callback=dml_Load_Map";
	script.id = "dmlMapApi";
...
// 3.5 - Creates new temporary pin on the map
...
// 3.7	- Sets the map on all markers in the array.
...
// 3.9	- Creates new marker on the map
...
function dml_Save_Settings() {
	...
	if (mySaveType == "Save Settings") {
		...
		var myImageLink = dmlMarkerImgVidLinkHelper(jQuery("#dmlMarkerImageValue").val());
		var myVideoLink = dmlMarkerImgVidLinkHelper(jQuery("#dmlMarkerVideoValue").val());
		var myMarkerLinkText = dmlMarkerLinkHelper(jQuery("#dmlMarkerLinkValue").val());
		var myMarkerLinkUrl = dmlMarkerLinkHelper(jQuery("#dmlMarkerLinkUrlValue").val());
...
// 5.2  - Activates text field for the Map Style code
...
// 5.3	- Clears all markers from the map
...
// 5.4	- Sets icon ID when a new icon selected for the marker
...
// 5.5  - Sets icon container ID when a new icon selected for the marker
...
// 5.7	- Calls AJAX to delete selected marker from the database
...
// 5.8	- Solves Lat data of the shape
...
// 5.9	- Solves Lng data of the shape
